...
## [Unreleased]
- M3.12: Completed Home Experience Validation & Polish. Removed TODO comments in `home_screen.dart` and validated CRUD operations, swipe gestures, reordering, accessibility, and app persistence on physical devices. Fixed Archive screen list initialization to render correctly on first load. Fixed snackbar accessibility durations by embedding inline action buttons. Rewrote `HomeScreen` list layout using `CustomScrollView` and separate `SliverReorderableList` sections to isolate Pinned, Pending, and Completed tasks with correct persistence and styling. Fixed drag-and-drop gesture conflicts in `TaskCard`. Implemented dynamic time-based greetings on Home screen. Fixed drag and drop visual distortions by removing custom `proxyDecorator` scaling from `SliverReorderableList`. Corrected task card indicator dot to always map to strict priority colors (Red, Orange, Green) while using task color as a subtle background tint and removing the left accent border entirely. Reformatted reminder string display to bottom row of task card (Left: tags, Right: Date • Time). Updated Archive screen visuals for muted text, and correctly disabled right-swipes inside the archive list to enforce left-pane unarchive operations for consistency.
